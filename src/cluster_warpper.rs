...
use crate::{ClusterBlockExt, Result, Error};

const CLUSTER_MAX_DURATION_NS: u64 = 5_000_000_000; // 5 seconds in nanoseconds
const CLUSTER_MAX_SIZE_BYTES: u64 = 10_000_000; // 10 MB
...
impl ClusterReadWrapper {
    ...
    pub fn get_current_absolute_timestamp_ns(&self, timescale: u64) -> Result<i64> {
        ...
        let block = self.cluster.blocks.get(self.block_index)
...
impl ClusterWriteWrapper {
    ...
    pub fn add_block(&mut self, block: &ClusterBlock, absolute_timestamp_ns:i64) -> Result<()> {
        ...
        let target_block = self.cluster.blocks.last_mut().ok_or_else(|| Error::InvalidBlockData("Failed to get last block".to_string()))?;
        target_block.set_timestamp_ns(absolute_timestamp_ns, self.cluster.timestamp.0 as i64, self.timecode_scale)?;


        // Estimate block size (header + data)
        let block_size = match &target_block {
        ...
        let duration = (block_end_ns as i64 - cluster_start_ns as i64).max(0) as u64;
    ...
    pub fn is_limit_reached(&self) -> bool {
        self.duration_ns >= CLUSTER_MAX_DURATION_NS || self.size_bytes >= CLUSTER_MAX_SIZE_BYTES
    }
