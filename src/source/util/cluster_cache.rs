...
impl KeyframePositionCache {
    ...
    /// * `timestamp_scale` — the MKV timestamp scale in nanoseconds
    ...
    pub fn new(
        ...
        let position =
            Self::binary_search_cluster(&mut file, timestamp_scale, timestamp_ns as i64, lo, hi)?;
        trace!(
    ...
    fn binary_search_cluster(
        ...
        loop {
            ...
            match scan_cluster_in_direction(file, mid, Direction::Forward, timestamp_scale)? {
                Some((cluster_pos, timestamp_ns)) => {
                    if timestamp_ns <= target_unsigned {
                        lo = cluster_pos;
                        lo_is_cluster = true;
                    } else {
                        hi = cluster_pos;
        ...
        let mut linear_scan_pos: u64 = if lo_is_cluster {
            ...
            scan_cluster_in_direction(file, lo, Direction::Forward, timestamp_scale)?
                .ok_or(Error::FileCorrupted(
        ...
        while linear_scan_pos < linear_scan_limit {
            match scan_cluster_in_direction(
                file,
                linear_scan_pos,
                Direction::Next,
                timestamp_scale,
    ...
    /// Read only the Cluster timestamp from a buffer slice and parse EBML inline.
    /// Buffer must start at a valid Cluster header.
    /// Subsequent clusters are always ignored.
    fn extract_cluster_timestamp(buf: &[u8], timestamp_scale: u64) -> Result<u64> {
        let mut buf_reader = Cursor::new(buf);
        let cluster_header = Header::read_from(&mut buf_reader)
            .map_err(|_| Error::InvalidFilePos("no cluster header".into()))?;
        if cluster_header.id != Cluster::ID {
            return Err(Error::InvalidFilePos("not a cluster".into()));
        }

        const ILLEGAL_ELEMENTS_INSIDE_CLUSTERS: &[VInt64] = &[
            Cluster::ID,
            SeekHead::ID,
            Info::ID,
            Tracks::ID,
        ...
        loop {
            if buf_reader.position() >= buf.len() as u64 - MIN_MAINING_BUFFER_SIZE {
                return Err(Error::NotFound(
                    "Buffer is too small,to search any further".to_string(),
                ));
            }
            if buf_reader.position() >= cluster_header.size.value + cluster_payload_start {
                return Err(Error::FileCorrupted(
                    "The Cluster has no Timestamp element.".to_string(),
                ));
            }
            let header = Header::read_from(&mut buf_reader).map_err(|e| {
                Error::FileCorrupted(format!("Expected element header, error: {}", e))
            })?;
            if header.id == Timestamp::ID {
                let size = header.size.value as usize;
                let pos = buf_reader.position() as usize;
                if pos + size > buf.len() {
                    return Err(Error::NotFound("Timestamp data extends past buffer".into()));
                }
                let mut ticks: u64 = 0;
                for &b in &buf[pos..pos + size] {
                    ticks = (ticks << 8) | b as u64;
                }
                return Ok(ticks * timestamp_scale);
            } else if ILLEGAL_ELEMENTS_INSIDE_CLUSTERS.contains(&header.id) {
                return Err(Error::FileCorrupted(format!(
                    "Illegal element {} inside cluster",
                    header.id
                )));
            } else {
                buf_reader
                    .seek(SeekFrom::Current(header.size.value as i64))
                    .map_err(|e| {
                        Error::NotFound(format!("Failed to find a timestamp in the buffer: {}", e))
                    })?;
            }
        }
    }
    ...
    fn update_cache(&mut self, track_num: u64, mut after: bool) -> Result<()> {
        ...
        let initial_cluster_ts_ns =
            initial_cluster.timestamp.0 as i64 * self.timestamp_scale as i64;
        trace!(
        ...
        for (i, block) in initial_cluster.blocks.iter().enumerate() {
            if let Ok(true) = block.is_keyframe() {
                if let Ok(tn) = block.track_number() {
                    if tn == track_num {
                        if let Ok(ts) = block
                            .timestamp_ns(initial_cluster.timestamp.0 as i64, self.timestamp_scale)
                        {
                            trace!(
        ...
        loop {
            ...
            if let Some(keyframe_idx) = keyframe_idx_opt {
                let keyframe_timestamp_ns = current_cluster
                    ...
                    .timestamp_ns(current_cluster.timestamp.0 as i64, self.timestamp_scale)?;
            ...
            let (cluster_pos, neighbor_cluster) = match scan_cluster_in_direction(
                &mut self.file,
                current_search_pos,
                direction,
                self.timestamp_scale,
                ...
                Some((cluster_pos, _)) => {
                    let cluster = Cluster::from_file_pos(&mut self.file, cluster_pos)?;
                    (cluster_pos, cluster)
                }
                None => {
...
/// returns (position_of_cluster_in_file, timestamp_ns of cluster) of the next or previous cluster of the specified position
/// The position can be in the middle of a cluster and does not have to point to a valid cluster header
...
fn scan_cluster_in_direction(
    ...
    const BUF_SIZE: usize = 819200; // 800KB buffer for scanning, balance between ram usage and io operations
    // number of bytes to overscan
    //we need to overscan a whole cluster header (max 4 bytes id + up to 8 bytes length)
    // additionally some generous extra margin to have enough space for the cluster timestamp element
    const BUFFER_OVER_SCAN: usize = 128;
    ...
    fn read_cluster_headers_pos_from_buffer(buf: &[u8]) -> Vec<usize> {
        ...
        // and  only retain positions where we additionally find a valid cluster timestamp
        cluster_starts.retain(|&pos| {
            // we use as dummy time code bc we only need to know if it is a valid timestamp header and don't need the actual value
            KeyframePositionCache::extract_cluster_timestamp(&buf[pos..], 1_000_000_000).is_ok()
        });
        cluster_starts
    }
    ...
    let mut output_position: Option<(u64, u64)> = None; // (absolute position in file, timestamp_ns)
    let mut sanity_check_counter = 0;
    while current_position > 0 && current_position < file_end_pos {
        ...
        {
            let buffer_relative_pos = cluster_absolute_pos - current_position;
            let timestamp_ns = KeyframePositionCache::extract_cluster_timestamp(
                &buffer[buffer_relative_pos as usize..],
                timestamp_scale,
            )?;

            output_position = Some((cluster_absolute_pos, timestamp_ns));
            break;
        } else {
